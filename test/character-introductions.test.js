...
test('a character the script never described gets null, not a guess', () => {
  ...
  assert.deepEqual(
    hearing.characters.map((character) => character.introduction),
    [null, null, null],
  );
});
...
test('a cue extension in the action is not a description', () => {
  const parsed = parseFountainScript(
    ['INT. VAULT - NIGHT', '', 'KIRA (V.O.) sighs offscreen.', '', 'KIRA', 'I am still here.'].join('\n'),
  );
...
test('page furniture between rows does not break an introduction', async () => {
  ...
  assert.equal(introOf(parsed, 'INSPECTOR BARKER').text, '40s, sharp British tweed, keen observant eyes');
});
