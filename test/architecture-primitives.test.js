...
import { createMutationQueue } from '../src/utils/staged-mutation.js';
...
test('latest operation aborts its predecessor and rejects stale commits even when work ignores abort', async () => {
  ...
  const firstRun = operation.run(async ({ signal }) => {
    firstSignal = signal;
...
test('bounded reader refuses a non-streaming body without buffering it', async () => {
  ...
  const response = {
    ...
    async blob() { blobCalls++; return new Blob([new Uint8Array(100)]); }
  };
  await assert.rejects(
    readBoundedResponseBlob(response, { maxBytes: 10 }),
    /cannot safely read/
  );
...
test('focus-preserving renderer restores live values, selection, scroll, and a deterministic fallback', () => {
  ...
  try {
    ...
    renderer.render(() => { root.innerHTML = view(true); });
...
test('staged mutation honors abort before commit and finishes metadata after the durable boundary', async () => {
  ...
  const beforeEvents = [];
  await assert.rejects(runStagedMutation({
    ...
    async commitDurable() { beforeEvents.push('commit'); },
    async persistMetadata() { beforeEvents.push('metadata'); },
    async cleanupStage() { beforeEvents.push('cleanup'); }
  }), error => error?.name === 'AbortError');
  ...
  const result = await runStagedMutation({
    ...
    async stage() { afterEvents.push('stage'); return 'staged'; },
    async commitDurable() {
      afterEvents.push('commit');
      afterCommit.abort();
      return 'durable';
    },
    async persistMetadata() { afterEvents.push('metadata'); return 'saved'; },
    async reconcile() { afterEvents.push('reconcile'); },
    async cleanupStage() { afterEvents.push('cleanup'); }
  });
...
test('staged mutation compensates metadata failure and preserves the primary error', async () => {
  ...
  const events = [];
  await assert.rejects(runStagedMutation({
    async stage() { events.push('stage'); return 'staged'; },
    async commitDurable() { events.push('commit'); return 'durable'; },
    async persistMetadata() { events.push('metadata'); throw failure; },
    async compensateDurable() { events.push('compensate'); },
    async cleanupStage() { events.push('cleanup'); }
  }), error => error === failure);
...
test('mutation queue serializes overlapping mutations and continues after failure', async () => {
  ...
  const secondRun = queue.run(async () => { events.push('second'); return 'saved'; });
  await Promise.resolve();
