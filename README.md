...
* **Bundled Voice Catalog**: Cast from audiobook narrators that ship with the app — searchable by vocal register and pace, both measured from the audio itself. The clips come from the [LibriTTS-R](https://www.openslr.org/141/) corpus under CC BY 4.0, so browsing needs no account, no API key, and no network.
